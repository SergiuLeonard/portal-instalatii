...
export default function VasExpansiune() {
  ...
  const [presiuneStatica, setPresiuneStatica] = useState(20); // mH2O
  ...
  const calcule = useMemo(() => {
    ...
    const deltaT = tempTur - 10; // Presupunem temperatura de umplere ~10°C
    const coefDilatare = 0.00045 * deltaT + 0.000002 * Math.pow(deltaT, 2);
    
    const volumDilatare = volumInstalatie * coefDilatare; // litri
    
    // Formula simplificată vas expansiune
    // Vvas = Vd × (Pm + 1) / (Pm - Pi) × fs
    // Unde Pm = presiune maximă, Pi = presiune minimă (statică)
    // +1 pentru compensarea presiunii atmosferice
    const Pm = presiuneMaxima / 10; // în bar
    const Pi = presiuneStatica / 10; // în bar
    ...
    return {
      ...
      recomandare: Math.ceil(volumVas / 50) * 50, // Rotunjim la 50L
    };
  }, [volumInstalatie, tempTur, tempRetur, presiuneStatica, presiuneMaxima]);

  // Estimare volum instalație bazată pe putere
  const estimareDinPutere = (kW: number) => {
    // Aproximativ 10-15 litri per kW pentru instalații de încălzire
    const litri = kW * 12;
